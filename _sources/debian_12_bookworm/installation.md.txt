...
- Debian 12 (Bookworm) with xfce4 desktop (recommended)
...
packages for MonoKrom Plasma on Debian 12. This is the simplest install path.
...
Download and install LinuxCNC from the official deb package for your architecture:

**AMD64 (PCs):**

```bash
cd ~/Downloads
sudo apt install ./linuxcnc-uspace_*.deb
```
...
If LinuxCNC is not yet available as a `.deb` for Bookworm, install it from the LinuxCNC
apt repository first. See [linuxcnc.org](https://linuxcnc.org) for details.
